...
from core import github_sync, local_state, scheduled_run
...
class ConfigDefaultsTests(unittest.TestCase):
    ...
    def test_schedule_defaults_use_ninety_and_one_eighty_minutes(self):
        with self.CONFIG_TEMPLATE.open("r", encoding="utf-8-sig") as file:
            config = json.load(file)
        self.assertEqual(90, config["SCHEDULE_BUSY_INTERVAL_MINUTES"])
    ...
    def test_runtime_fallback_matches_latest_mainland_defaults(self):
        ...
        tree = ast.parse(
            (PROJECT_ROOT / "core" / "app.py").read_text(encoding="utf-8-sig")
        )
        defaults = None
...
class SetupScriptTests(unittest.TestCase):
    ...
    def test_windows_setup_uses_venv_retries_and_real_exit_codes(self):
        ...
        self.assertIn('"config\\config.example.json"', script)
        self.assertIn('"scripts\\update_fork.ps1"', script)
        self.assertIn('$SchedulerModule = "core.scheduled_run"', script)
        self.assertIn('"-X utf8 -m $SchedulerModule"', script)
        self.assertIn('$env:ComSpec', script)
        self.assertIn('cd /d `"`"$WorkingDirectory`"`"', script)
        self.assertNotIn('$LegacyPythonScriptPath', script)
        ...
        self.assertIn('"core\\proxy_scoring.py"', script)
    ...
    def test_setup_is_the_single_entrypoint_for_safe_updates(self):
        ...
        windows_update = (PROJECT_ROOT / "scripts" / "update_fork.ps1").read_text(
            encoding="utf-8-sig"
        )
        linux_setup = (PROJECT_ROOT / "setup.sh").read_text(encoding="utf-8")
        linux_update = (PROJECT_ROOT / "scripts" / "update_fork.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_scheduler_enables_utf8_for_child_process(self):
        script = (PROJECT_ROOT / "core" / "scheduled_run.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_linux_setup_uses_venv_and_preserves_gitignore(self):
        ...
        self.assertIn("ENABLE_SCHEDULED_TASK", script)
        self.assertIn('CONFIG_TEMPLATE_PATH="$SCRIPT_DIR/config/config.example.json"', script)
        self.assertIn('-m core.scheduled_run', script)
        self.assertIn('dir "/scheduled_run.py"', script)
        self.assertNotIn('! -f scheduled_run.py', script)
        self.assertNotIn('scripts/update_fork.sh update_fork.sh', script)
    ...
    def test_linux_updater_resolves_and_validates_the_parent_repository(self):
        script = (PROJECT_ROOT / "scripts" / "update_fork.sh").read_text(
            encoding="utf-8"
        )
        self.assertIn('PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"', script)
        self.assertIn('cd "$PROJECT_ROOT"', script)
        self.assertIn('[[ ! -d $PROJECT_ROOT/.git ]]', script)
        self.assertIn('[[ ! -f $PROJECT_ROOT/main.py ]]', script)
        self.assertIn('[[ ! -f $PROJECT_ROOT/config/config.example.json ]]', script)
...
class GitHubSyncTests(unittest.TestCase):
    def test_manual_sync_uses_configured_local_output_by_default(self):
        script = (PROJECT_ROOT / "core" / "github_sync.py").read_text(
            encoding="utf-8"
        )
        self.assertIn(
        ...
        for name in ("git_sync.ps1", "git_sync.sh"):
            wrapper = (PROJECT_ROOT / "scripts" / name).read_text(
                encoding="utf-8-sig"
            )
            self.assertIn("core.github_sync", wrapper)
...
class ScheduleTests(unittest.TestCase):
    CONFIG = {
        ...
        "SCHEDULE_BUSY_INTERVAL_MINUTES": 90,
        "SCHEDULE_OFFPEAK_INTERVAL_MINUTES": 180,
    }

    def test_busy_period_runs_every_ninety_minutes_from_eighteen_hundred(self):
        expected = {0: True, 15: False, 30: False, 45: False, 90: True}
        for minute, should_run in expected.items():
            now = datetime(2026, 7, 13, 18 + minute // 60, minute % 60, tzinfo=timezone.utc)
            self.assertEqual(
                (should_run, True, 90), scheduled_run.should_run(now, self.CONFIG)
            )
    ...
    def test_schedule_uses_the_expected_daily_time_grid(self):
        expected_run_minutes = {
            0, 180, 360, 540, 720, 900,  # 00:00–15:00, every three hours
            1080, 1170, 1260, 1350,  # 18:00–22:30, every ninety minutes
        }
        for minute in range(0, 24 * 60, 15):
            now = datetime(2026, 7, 13, minute // 60, minute % 60, tzinfo=timezone.utc)
            expected_busy = minute >= 18 * 60
            expected_interval = 90 if expected_busy else 180
            self.assertEqual(
                (minute in expected_run_minutes, expected_busy, expected_interval),
                scheduled_run.should_run(now, self.CONFIG),
            )

    @mock.patch("core.scheduled_run.subprocess.call")
    @mock.patch(
        "core.scheduled_run.load_config",
