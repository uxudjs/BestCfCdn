...
配置文件：仓库根目录 config.json（规范模板为 config/config.example.json）
...
def load_config():
    ...
    try:
        ...
        print("请运行 setup，或从 config/config.example.json 创建根目录 config.json。")
        return defaults
...
def sync_to_github():
    ...
    module_path = os.path.join(os.path.dirname(CONFIG_FILE), "core", "github_sync.py")
    if not os.path.exists(module_path):
        print("未找到 core.github_sync，跳过 GitHub 同步。")
        return
...
def _run():
    ...
    token_file = os.path.join(os.path.dirname(CONFIG_FILE), IP_CALIBRATION_TOKEN_FILE)
    cache_file = os.path.join(os.path.dirname(CONFIG_FILE), IP_CALIBRATION_CACHE_FILE)
